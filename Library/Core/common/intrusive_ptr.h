...
constexpr uint64_t kImpracticallyHugeWeakReferenceCount = (kImpracticallyHugeReferenceCount << 32);
...
{
    return detail::refcount(
        atomic_combined_refcount_increment(combined_refcount, kReferenceCountOne));
}

inline uint32_t atomic_weakcount_increment(std::atomic<uint64_t>& combined_refcount)
{
    return detail::weakcount(
        atomic_combined_refcount_increment(combined_refcount, kWeakReferenceCountOne));
}
...
{
    return detail::weakcount(
        atomic_combined_refcount_decrement(combined_refcount, kWeakReferenceCountOne));
}
...
//      intrusive_ptr_target.");
...
#endif
    static_assert(
        std::is_base_of_v<TTarget, std::remove_pointer_t<decltype(NullType::singleton())>>,
    ...
    void reset_() noexcept
    {
        if (target_ != NullType::singleton())
        {
            ...
            {
                bool should_delete = (combined_refcount == detail::kWeakReferenceCountOne);
    ...
    explicit intrusive_ptr(TTarget* target) : intrusive_ptr(target, raw::DontIncreaseRefcount{})
    {
        if (target_ != NullType::singleton())
        {
            ...
            target_->combined_refcount_.store(detail::kUniqueRef, std::memory_order_relaxed);
        }
    }
...
public:
    ...
    explicit intrusive_ptr(TTarget* target, raw::DontIncreaseRefcount /*unused*/) noexcept
        : target_(target)
    {
    }
    ...
    template <class From, class FromNullType>
    intrusive_ptr& operator=(const intrusive_ptr<From, NullType>& rhs) & noexcept
    {
        static_assert(
            std::is_convertible_v<From*, TTarget*>,
    ...
    TTarget& operator*() const noexcept { return *target_; }
   ...
   */
    static intrusive_ptr unsafe_steal_from_new(TTarget* raw_ptr) { return intrusive_ptr(raw_ptr); }
   ...
   */
    static intrusive_ptr unsafe_adapt_non_heap_allocated(
        TTarget* raw_ptr, uint32_t expected_decrefs)
    {
        ...
        result.target_->combined_refcount_.store(
            detail::refcount(detail::kImpracticallyHugeReferenceCount + expected_decrefs) |
...
public:
    ...
    weak_intrusive_ptr(const weak_intrusive_ptr& rhs) : target_(rhs.target_) { retain_(); }
    ...
    template <class From, class FromNullType>
    weak_intrusive_ptr& operator=(const weak_intrusive_ptr<From, NullType>& rhs) & noexcept
    {
        static_assert(
            std::is_convertible_v<From*, TTarget*>,
    ...
    //
    TTarget* _unsafe_get_target() const noexcept { return target_; }
    ...
    uint32_t use_count() const noexcept
    {
        ...
        return target_->refcount(std::memory_order_relaxed);  // refcount, not weakcount!
    }
    ...
    intrusive_ptr<TTarget, NullType> lock() const noexcept
    {
        ...
        {
            auto combined_refcount = target_->combined_refcount_.load(std::memory_order_relaxed);
            do
            {
                if (detail::refcount(combined_refcount) == 0)
                {
